...
bool Motion::DoMotion( MotionType motionType )
{
	...
	if( mWorldModel->mIsFalled )
	{
		...
		else if( lastGetUpActionDone )
		{
			...
			getUpNum ++;
			bool reverseGetUp = ( getUpNum % 10 >= countGetUpNum  );	// getUpNum % 8 == 0 - 4 :正常起立
																	// getUpNum % 8 == 5 - 7 :反向起立
...
bool Motion::HMotionBlock()
{
	Vector3f selfPos = mWorldModel->mSelf.mPos;
	Vector3f ballPos = mWorldModel->mBall.mPos;
	float ActionSpeedRate =1.4f;//我方球员的反应系数
...
bool Motion::HMotionRunPosition()
{
	...
	Vector3f safePos;													    //不被弹出的与球保持一定距离的位置
	float tempDist = 1.25f;													//初始为1
	float shouldMoveDist = 0.5f;											//站位的0.4米内有队友需站在另一位置,f
...
bool Motion::HMotionIntercept()
{
	...
	//	currInterceptPos.x-=0.6f;//第二个抢断队员站位相对更靠后
...
bool Motion::HMotionClearBall()
{
	...
	float ActionSpeedRate =1.45f;//我方球员的反应系数
...
bool Motion::MotionDeadBallStand()
{
	...
	bool ourKick;
	unsigned int caseNum = 0;	// 1 -- 我方发界外球或角球		--- dribble
								// 2 -- 对方发界外球			--- wait
								// 3 -- 对方发门球				--- wait
								// 4 -- 对方中场开球			--- wait
								// 5 -- 我方发门球				--- dribble
